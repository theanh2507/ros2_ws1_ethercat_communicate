  ...
  size_t current_alignment)
{
  ...
  if (is_plain) {
    ...
    using DataType = ethercat_msgs__srv__SetSdo_Request;
    is_plain =
      (
      offsetof(DataType, sdo_value) +
      last_member_size
      ) == ret_val;
  }
  ...
  size_t current_alignment)
{
  ...
  if (is_plain) {
    ...
    // We still need to check that the in-memory alignment
    // is the same as the CDR mandated alignment.
    using DataType = ethercat_msgs__srv__SetSdo_Response;
    is_plain =
      (
      offsetof(DataType, sdo_return_message) +
      last_member_size
      ) == ret_val;
  }
